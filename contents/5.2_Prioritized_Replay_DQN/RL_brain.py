...
# 设置随机种子
np.random.seed(1)
tf.random.set_seed(1)  # 替换原来的 tf.set_random_seed(1)
...
class DQNPrioritizedReplay:
    ...
    def _build_net(self):
        ...
        # 在 loss 计算部分，将 tf.squared_difference 替换为等价操作
        with tf.compat.v1.variable_scope('loss'):
            if self.prioritized:
                # 使用平方差作为损失函数
                self.abs_errors = tf.abs(self.q_target - self.q_eval)
                self.loss = tf.reduce_mean(self.ISWeights * tf.square(self.q_target - self.q_eval))
            else:
    ...
    def learn(self):
        ...
        if self.prioritized:
            ...
            # 确保 abs_errors 是一维数组
            abs_errors = np.abs(abs_errors).reshape(-1)
